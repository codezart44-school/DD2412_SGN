...
class GCELoss(nn.Module):
    def __init__(self, num_classes, q=0.7):
        ...
        Args:
            ...
            q: GCE hyperparameter controlling the robustness.
        """
    ...
    def forward(self, logits, labels):
        """
        Args:
            logits: Model predictions before softmax, shape [B, num_classes].
            labels: Ground truth labels, shape [B].
        Returns:
            Loss value (scalar).
        """
        probs = F.softmax(logits, dim=1)
        probs_y = probs.gather(dim=1, index=labels.unsqueeze(1)).squeeze(1)  # [B]
        if self.q == 1.0:
            # Standard Cross-Entropy Loss
            loss = -torch.log(probs_y + 1e-12).mean()
        else:
            # GCE Loss
            loss = (1.0 - probs_y**self.q) / self.q
            loss = loss.mean()
        return loss
...
def train_gce(
    ...
    # Replace nn.CrossEntropyLoss with GCELoss
    q = 0.7  # GCE hyperparameter, adjust as needed
    criterion = GCELoss(num_classes=10, q=q)  # NOTE
